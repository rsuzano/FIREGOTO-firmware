...
//Modified by Regis Costa based on 
// ESP32 WiFi <-> 3x UART Bridge
// by AlphaLima
// www.LK8000.com
...
#define SERIAL0_TCP_PORT 9999       // Wifi Port UART0
/*************************  COM Port 1 *******************************/
...
void setup() {
  ...
  FSInfo fs_info;
  SPIFFS.info(fs_info);
  str += "TotalKBytes=> ";str +=String(fs_info.totalBytes/1024);str +="\r\n";
  str += "usedKBytes=> ";str +=String(fs_info.usedBytes/1024);str +="\r\n";
  str += "blockSize=> ";str +=String(fs_info.blockSize);str +="\r\n";
  str += "pageSize=> ";str +=String(fs_info.pageSize);str +="\r\n";
  str += "maxOpenFiles=> ";str +=String(fs_info.maxOpenFiles);str +="\r\n";
  str += "maxPathLength=> ";str +=String(fs_info.maxPathLength);str +="\r\n";
  COM[DEBUG_COM]->print(str);
  ...
  .setCacheControl("max-age=6");
  webServer.onNotFound(notFound);
  webServer.on("/command", HTTP_GET, [](AsyncWebServerRequest * request) {
    String cmd;
    int HTTP_STATUS = 200;
    int COM_PORT = 0;
    String retCmd = "";
    if (request->hasParam("comPort")) {
      COM_PORT = request->getParam("comPort")->value().toInt();
    }
    if (request->hasParam("cmd")) {
      cmd = request->getParam("cmd")->value();
      COM[COM_PORT]->println(cmd);

      if (COM[COM_PORT]->available()) {
        while (COM[COM_PORT]->available())
        {
          retCmd += (char)Serial.read(); // read char from UART(num)
        }
      }
    } else {
      retCmd = "No cmd sent";
    ...
    request->send(HTTP_STATUS, "text/plain", retCmd);
  });
